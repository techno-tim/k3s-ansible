...
## ⚙️ Key takeaways deploying k3s latest releases after May/2022


###1) make sure to have different hostnames for your servers in the cluster.
exemple for each host run:
```bash
...
###2) k3s Auto Deploy on boot directory is /var/lib/rancher/k3s/server/manifests whtever yaml we throw here will be deployed.
There is one caveat though, filenames cant contain dots before the dotted yaml file extension.
example.
```bash
/var/lib/rancher/k3s/server/manifests/metallb.namespace.yaml  #will not work and cause this namespace to be an infinite deletion/recreate loop.
```
Make sure to rename it to something else that doesnt have dots before extension name.:
```bash
/var/lib/rancher/k3s/server/manifests/metallb-namespace.yaml
```

Repeat the fix for the remaning dotted named files you have.
